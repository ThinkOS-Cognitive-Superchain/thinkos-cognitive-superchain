...
struct Weights { w0: f64, w1: f64, w2: f64, w3: f64, w4: f64 }
#[derive(Debug, Serialize, Deserialize)]
struct VaultSplit { innovation: u32, governance: u32 }
...
fn post_weights(aifa: &str, tele: &Telemetry) -> anyhow::Result<Weights> {
    ...
    if !resp.status().is_success() { anyhow::bail!("AIFA /weights {}", resp.status()); }
...
fn get_vault_split(aifa: &str, mode: &str) -> anyhow::Result<VaultSplit> {
    ...
    if !resp.status().is_success() { anyhow::bail!("AIFA /vault_split {}", resp.status()); }
...
fn banner(node_id: &str) {
    println!("{}", "================================================".bright_cyan());
    println!("{}", format!(" ThinkOS Cognitive Superchain — Node {}", node_id).bright_cyan().bold());
    println!("{}", " (CMPS + AIFA + Loop + Per-node persistence)".bright_cyan());
    println!("{}", "================================================".bright_cyan());
}
...
fn main() -> anyhow::Result<()> {
    ...
    let out_dir     = std::env::var("THINKOS_STATE_DIR").unwrap_or_else(|_| format!("state/nodes/{node_id}"));
    ...
    println!("Node: {}  Out: {}  Loop: {}s  Iters: {}", node_id, out_dir, loop_secs, iters);
    println!("Telemetry: vol={:.3} cong={:.3} upvar={:.3} treas={:.3}",
        tele.volatility, tele.congestion, tele.uptime_variance, tele.treasury_health);

    let s = thinkos_cmps::Scores { continuity: 0.90, cognition: 0.80, synergy: 0.70, adaptation: 0.60, integrity: 0.95 };

    for i in 1..=iters {
        let ts: DateTime<Utc> = Utc::now();
        print!("{}", format!("[{ts}] [{}] tick {:03}  ", node_id, i).bright_black());

        // fetch weights
        match post_weights(&aifa_url, &tele) {
            Ok(w) => {
                let score = composite(&s, &w);
                println!("{}", format!("weights=({:.3},{:.3},{:.3},{:.3},{:.3}) score={:.4}",
                    w.w0, w.w1, w.w2, w.w3, w.w4, score).white());
    ...
    println!("{}", format!("Node {} exiting after {} ticks.", node_id, iters).bright_green());
    Ok(())
}
